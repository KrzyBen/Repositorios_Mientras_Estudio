...
const [aviso, error] = await crearAvisoEncargado(data);
...
const [aviso, error] = await validarAviso(id, apropiado, aprobado);
...
export async function ocultar(req, res) {
try {
const { id } = req.params;
const [aviso, error] = await ocultarAviso(id);
if (error) return res.status(404).json({ error });
